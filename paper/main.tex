...
learnable parameters. DeFT can be inserted into existing frozen
vision-language segmentation runtimes without retraining. Experiments on standard open-vocabulary
...
implementation, we insert DeFT after the frozen visual encoder produces patch
...
by a vision-language segmentation runtime, where $N=H \times W$. Let $T \in
...
\paragraph{Runtime.}
We use a frozen CLIP-based open-vocabulary segmentation runtime as the base
pipeline. DeFT is inserted after visual token extraction and before text-image similarity.
All runtime settings, prompts, and post-processing are kept unchanged unless
explicitly stated.
